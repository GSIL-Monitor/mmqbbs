package com.pay;/**
 * 描述：
 * Created by Liuyg on 2018-12-31 0:45
 */
...
/*	private static boolean isValidChar(char ch) {
     ...
     */
    public String sendRedPack(String mch_id,String appid, String opendid,String send_name,String Wishing,String zfKey,String Client_ip,String Act_name,String Remark,String gzhName,String totalAmount) throws KeyStoreException, NoSuchAlgorithmException, IOException,
        ...
        UUIDHexGeneratorutil uuidGenerator = new UUIDHexGeneratorutil();
        ...
        sendRedPackPo.setSend_name(send_name);//商户名称
        sendRedPackPo.setRe_openid(opendid);
        sendRedPackPo.setTotal_amount(totalAmount);
        sendRedPackPo.setTotal_num("1");
        sendRedPackPo.setWishing(Wishing);//红包祝福语
        sendRedPackPo.setClient_ip(Client_ip); //调用接口的机器Ip地址
        sendRedPackPo.setAct_name(Act_name);//活动名称
        sendRedPackPo.setRemark(Remark);//备注信息
        ...
        String key = "&key="+zfKey; //商户支付密钥
        System.out.println(prestr+key);
        ...
        FileInputStream instream = new FileInputStream(new File(ResourceUtils.getFile("classpath:payzs/"+gzhName+"/apiclient_cert.p12").getPath())); //此处为证书所放的绝对路径
        try {
